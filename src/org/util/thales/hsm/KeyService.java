...
import org.util.thales.hsm.constants.KeyLength;
...
public class KeyService {
	...
	//@formatter:off
	public KeyService(final HSMService hsmService) {
		this.hsmService  = hsmService;
	}

	
	public final HSMResponse calculateKCV(final String key, final KeyLength keyLength, final KeyType keyType, final Logger logger) {
		try {
	 ...
	 */
	public final GenKeyResponse generateKey(final KeyType keyType, final KeyScheme.VARIANT keyScheme, final Logger logger) {
		try {
	 ...
	 */
	public final GenKeyResponse generateExportKey(final KeyType keyType, final VARIANT keyScheme, final ANSI keySchemeExport,
	 ...
	 * @return			: The new key, encrypted under the LMK.
	 */
	public final GenKeyResponse deriveIPEK(final String iksn, final BDKType bdkType, final String bdklmk, final Logger logger) {
		try {
	 ...
	 * @return			: The new key, encrypted under the LMK and provided master key.
	 */
	public final GenKeyResponse deriveExportIPEK(final String iksn, final BDKType bdkType, final String bdklmk, final MasterKeyType mKeyType, final String mKeyLmk, final Logger logger) {
		try {
			final String bdkMode = bdkType == BDKType.BDK1 ? "1" : "2";
	 ...
	 * @return
	 */
	public final GenKeyResponse formKeyFromComponents(final KeyType keyType, final KeyScheme keyScheme, final Logger logger, final String... keys) {
		try {
	 ...
	 */
	public final GenKeyResponse importKey(final KeyType keyType, final VARIANT importScheme, final String keyzmk, final String zmklmk, final Logger logger) {
		try {
	 ...
	 */
	public final GenKeyResponse exportKey(final KeyType sourceKeyType, final String keylmk, final ANSI exportScheme, final MasterKeyType mKeyType, final String mKeylmk,	final Logger logger) {
		try {
	...
	public static void main(String[] args) {
		...
		System.out.println(hsmService.key().calculateKCV("U869393350325267BA5CE86B9283A4291", KeyLength.DOUBLE_LENGTH, KeyType.ZMK, Logger.CONSOLE));;
		System.exit(0);
		final GenKeyResponse response = hsmService.key().exportKey(KeyType.MK_AC, "U6C58184333A1628A1EF4F0C3B9C41D37", ANSI.DOUBLE_LEN, MasterKeyType.ZMK,
